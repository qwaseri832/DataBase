...
Номер записи (LSN) выдаёт сам журнал — под тем же мьютексом, под которым
запись попадает в батч. К движку батч применяется сразу после `fsync`, в том
же порядке, в каком лёг в журнал, и до того, как разрешатся `Future`. Поэтому
состояние в памяти и содержимое журнала не расходятся, даже когда два клиента
одновременно пишут один ключ: порядок в обоих один и тот же.
...
|---|---|---|
| `engine` | `type` | in_memory (другого нет) |
| `engine` | `partitions` | 1 |
| `wal` | `batch_size` | 100 |
...
| `server` | `read_buffer` | 4KB |
| `server` | `max_message_size` | 64MB |
| `server` | `idle_timeout` | без ограничения |
| `replication` | `role` | без репликации |
| `replication` | `master_addr` | обязателен при заданной роли |
| `replication` | `sync_interval` | 1s |
| `logging` | `level` | info |
